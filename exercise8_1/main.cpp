...
{
    //Konsolenausgabe
    cout << "Aufgabe 8.1: Bildbereich füllen" << endl << endl;

    //ifstream Objekt erstellen
    ifstream eingabeDatei;

    //Datei öffnen
    eingabeDatei.open("kunst.pgm");
    ...
    //Datei öffnen
    ausgabeDatei.open("kunst.out.pgm");
    ...
    cout << "k Kantenbildung" << endl << "s Schärfung" << endl << "f füllen" << endl << "e ENDE" << endl;
    ...
    switch (inputChar)
    {
        ...
        case 's': schaerfen(einleseFeld, manipuliertesBild, zeilen, spalten, 255);
            break;
        case 'f':
            //Extrem eigenaratiger workaround da ja nur das Originalbild übergeben und manipuliert werden darf???? 
            for (int i = 0; i < zeilen; i++)
            {
                for (int j = 0; j < spalten; j++)
                {
                    manipuliertesBild[i][j] = einleseFeld[i][j];
                }
            }

            fill(manipuliertesBild, zeilen, spalten, 10, 10, 255, 127);
            break;
        case 'e': exit(0);
    }
 ...
 * übernimmt
 * 
  Übergabeparameter: bild1: original bild; bild2: manipuliertes bild; nz Zeilen; 
 * ns Spalten; inz zeile des geimpften pixels; ins spalten des geimpften pixels;
 * oldval alter grauwert; newval neuer grauwert
 */
void fill(Pixel bild[N][N], int nz, int ns, int inz, int ins, Pixel oldval, Pixel newval)
{
    if (bild[inz][ins] == oldval)
    {
        bild[inz][ins] = newval;
    } else
    {
